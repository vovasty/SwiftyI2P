...
void inline configure_i2p(I2PConfig cfg){
    i2p::config::SetOption("socksproxy.enabled", cfg.socksProxyEnabled != 0);
    i2p::config::SetOption("httpproxy.enabled", cfg.httpProxyEnabled != 0);
    i2p::config::SetOption("httpproxy.port", (uint16_t)cfg.httpProxyPort);
    i2p::config::SetOption("socksproxy.port", (uint16_t)cfg.socksProxyPort);
    i2p::config::SetOption("datadir", (std::string)cfg.datadir);
    i2p::config::SetOption("port", (uint16_t)cfg.port);
    if (cfg.host && cfg.host[0])
        i2p::config::SetOption("host", cfg.host);
    i2p::config::SetOption("floodfill", cfg.floodfill != 0);
    i2p::config::SetOption("loglevel", (std::string)cfg.loglevel);
}

int i2p_init(I2PConfig cfg) {
    char *argv[] = {(char*)"i2pd", NULL};
