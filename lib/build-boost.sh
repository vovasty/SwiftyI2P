...
: ${BOOST_LIBS:="filesystem program_options date_time system"}
...
: ${TARBALLDIR:=`pwd`/dist}
: ${SRCDIR:=`pwd`/build/boost/src}
: ${IOSBUILDDIR:=`pwd`/build/boost/libs/boost/lib}
: ${IOSINCLUDEDIR:=`pwd`/build/boost/libs/boost/include/boost}
: ${PREFIXDIR:=`pwd`/build/boost/ios/prefix}
: ${COMPILER:="clang++"}
...
restoreBoost()
{
    if [ -e $BOOST_SRC/tools/build/example/user-config.jam-bk ]; then
...
scrunchAllLibsTogetherInOneLibPerPlatform()
{
    ...
    for NAME in $BOOST_LIBS; do
        ...
        $ARM_DEV_CMD lipo -create \
            "iphone-build/stage/lib/libboost_$NAME.a" \
            "iphonesim-build/stage/lib/libboost_$NAME.a" \
