...
set -e
if [ ! -e $DIST_PATH/openssl-${VERSION}.tar.gz ]; then
  echo "Downloading openssl-${VERSION}.tar.gz"
  curl ${CURL_OPTIONS} https://www.openssl.org/source/openssl-${VERSION}.tar.gz > "${DIST_PATH}/openssl-${VERSION}.tar.gz"
else
...
do
  ...
  tar zxf "${DIST_PATH}/openssl-${VERSION}.tar.gz" -C "$src_work_dir"
...
lipo -create \
  ...
  -output ${LIB_PATH}/libcrypto-tvOS.a

mkdir -p ${INCLUDE_PATH}
cp -R ${CURRENTPATH}/bin/iPhoneSimulator${IOS_SDKVERSION}-x86_64.sdk/include/openssl ${INCLUDE_PATH}
