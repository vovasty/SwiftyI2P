...
verifyOpenssl()
{
    ...
    if [ $res == false ]; then
      echo "openssl tarball failed signature check"
      exit 1
    else
      echo "openssl does match: ${res}"
    fi
...
# verify tarball
echo "Verify openssl ${DIST_PATH}/openssl-${VERSION}.tar.gz  ......"
verifyOpenssl "$OPENSSL_SHA" "${DIST_PATH}/openssl-${VERSION}.tar.gz"
